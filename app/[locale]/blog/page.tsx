...
export async function generateMetadata({
  ...
  const title =
    locale === "ro"
      ? "Blog — Sfaturi & Video despre Asistența Medicală la Domiciliu | Medline.md"
      : "Блог — Советы и Видео о Медицинской Помощи на Дому | Medline.md";
  const description =
    locale === "ro"
      ? "Articole, sfaturi și clipuri video despre îngrijirea la domiciliu, perfuzii, îngrijirea vârstnicilor și sănătate, de la echipa Medline.md."
      : "Статьи, советы и видео об уходе на дому, капельницах, уходе за пожилыми и здоровье от команды Medline.md.";
  return {
    title,
    description,
    keywords:
...
export default async function BlogPage({
  ...
  const articles = await getBlogList(locale);
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {articles.map((art) => (
                ...
                >
                  <div className="relative flex h-40 items-center justify-center overflow-hidden bg-gradient-to-br from-brand-500 to-brand-700 text-white">
                    {art.cover ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={art.cover} alt={art.title} className="h-full w-full object-cover" />
                    ) : art.tiktok ? (
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <p className="mt-2 flex-1 text-sm leading-relaxed text-slate-600">{art.excerpt}</p>
